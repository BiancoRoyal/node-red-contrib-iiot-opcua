  ...
  function OPCUAIIoTFlexServer(this: OPCUAIIoTFlexServer, config: OPCUAIIoTFlexServerDef) {
    ...
    const initNewServer = () => {
      ...
      try {
        ...
        handleServerError(node, err, {payload: 'Flex Server Failure! Please, check the server settings!'})
      }
    }
    ...
    this.on('input', function (msg: Todo) {
      ...
      if (msg.injectType === 'CMD') {
        ...
        handleServerError(node, new Error('Unknown Flex Inject Type ' + msg.injectType), msg)
      }
    })
    ...
    const executeOpcuaCommand = (msg: Todo) => {
      if (msg.commandType === 'restart') {
        ...
        handleServerError(node, new Error('Unknown Flex OPC UA Command'), msg)
      }
    }
